...
const validateNewElement = function (elementId) {
    // Verificar si el ID del nuevo control ya existe en el formulario
    const existingElement = document.getElementById(elementId);
    if (existingElement) {
        alert("No es permitido controles con el mismo ID.");
        return false;
    }
    return true;
};
...
buttonAddElemento.onclick = () => {
    if (nombreElemento.value !== "" && tituloElemento.value !== "") {
        let elemento = cmbElemento.value;
        let elementId = `id${nombreElemento.value}`;

        if (validateNewElement(elementId)) {
            if (elemento === "select") {
                newSelect();
